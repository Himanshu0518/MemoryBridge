...
import {
  Eye, EyeOff, Mail, Lock, User, Loader2, CheckCircle2, XCircle,
} from "lucide-react";
        ...
        {strengthRules.map((rule) => {
          ...
          return (
            <li key={rule.label} className="flex items-center gap-1.5 text-xs">
              {ok
                ? <CheckCircle2 className="size-3 text-emerald-500 shrink-0" aria-hidden="true" />
                : <XCircle className="size-3 text-muted-foreground shrink-0" aria-hidden="true" />
              }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-1.5">
        ...
        <p className="text-sm text-muted-foreground">
          Start preserving your memories with MemoryBridge
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
        <FormField label="Full name" error={errors.fullName?.message} htmlFor="fullName" required>
          <Input
            id="fullName"
            type="text"
            autoComplete="name"
        ...
        <div className="space-y-1">
          <div className="flex items-start gap-2">
            <input
              ...
              className={cn(
                "mt-0.5 size-4 shrink-0 cursor-pointer rounded border border-input bg-background accent-primary",
          ...
          {errors.agreeToTerms && (
            <p className="ml-6 text-xs text-destructive" role="alert" aria-live="polite">
        ...
        <Button
          ...
          size="lg"
          disabled={isLoading}
          className={cn("w-full mt-2", isLoading && "cursor-not-allowed")}
        >
          {isLoading ? (
            <>
